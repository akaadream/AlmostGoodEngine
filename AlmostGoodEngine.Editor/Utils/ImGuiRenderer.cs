...
{
	/// <summary>
	/// ImGUI renderer for ImGUI usage with MonoGame
	/// Stolen from https://github.com/Mezo-hx/MonoGame.ImGuiNet
	/// and https://github.com/isadorasophia/murder/blob/main/src/Murder.Editor/Utilities/Gui/ImGuiRenderer.cs
	/// </summary>
	public class ImGuiRenderer : IDisposable
	{
		...
		public ImGuiRenderer(Game game, params string[] fonts)
		{
			// The imnodes library should be initialized first
			//ImNodes.Initialize();

			// The context of imgui (and shared with a the libraries)
		...
		void AddFonts()
		{
			...
			unsafe
			{
				...
				{
					ImFontPtr? AddFont(string fontName, float size, ImFontConfigPtr configPtr, IntPtr r)
					{
						string path = Path.Join("fonts", fontName);
						if (!File.Exists(path))
						{
							Console.WriteLine($"The file {path} is unreachable!");
							return null;
						}
						Console.WriteLine($"The file {path} is loaded!");
		...
		public virtual IntPtr BindTexture(Texture2D texture)
		{
			...
			{
				Debugger.Break();
				Console.WriteLine($"{nameof(ImGuiRenderer)}: You have loaded {_loadedTextures.Count} textures. This may cause performance issues. Consider unloading unused textures.");
			}
		...
		{
			if (unloadPrevious && _loadedTextures.TryGetValue(id, out var tex))
			{
				tex.Dispose();
			}
